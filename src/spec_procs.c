...
SPECIAL(cleric_marduk)
{
  ...
  if ( (tmp == 7 ) || (tmp == 8) || (tmp == 9) || (tmp == 10)) {
    tmp = rand_number(1, num_used);
      ...
      if ((tmp == 5) && (GET_LEVEL(ch) > 4 && affected_by_spell(ch, SPELL_CURSE))) {
      ...
      if (tmp == 11) {
        cast_spell(ch, vict, NULL, SPELL_CAUSE_LIGHT);
        return TRUE;
      }
      if (tmp == 12 && GET_LEVEL(ch) > 8) {
        cast_spell(ch, vict, NULL, SPELL_CAUSE_CRITIC);
        return TRUE;
      }
  }
  return FALSE;
...
SPECIAL(cleric_ao)
{
  ...
  if ( (tmp == 7 ) || (tmp == 8) || (tmp == 9) || (tmp == 10)) {
    ...
    if ((tmp == 4) && (GET_LEVEL(ch) > 4 && affected_by_spell(ch, SPELL_CURSE))) {
      cast_spell(ch, ch, NULL, SPELL_REMOVE_CURSE);
      return TRUE;
    }
...
SPECIAL(magic_user)
{
  ...
  if (IS_NPC(ch) && GET_POS(ch) > POS_SITTING && GET_CLASS(ch) == CLASS_MAGIC_USER) {
  ...
  if ( (tmp == 8) || (tmp == 9) || (tmp == 10)) {
    ...
    if ((tmp == 2) && !affected_by_spell(ch, SPELL_ARMOR)) {
      cast_spell(ch, ch, NULL, SPELL_ARMOR);
      return TRUE;
    }
