...
cpp_extern const struct command_info cmd_info[] = {
  ...
  { "assedit"  , "asse"	, POS_STANDING, do_assedit  , LVL_IMPL, 0},
  { "assemble" , "ass"	, POS_SITTING , do_assemble , 0, SCMD_ASSEMBLE },
  ...
  { "autosac"  , "autosa", POS_DEAD    , do_gen_tog  , 0, SCMD_AUTOSAC },
  { "autosplit", "autosp"	, POS_DEAD    , do_gen_tog  , 0, SCMD_AUTOSPLIT },

  { "backstab" , "ba"	, POS_STANDING, do_backstab , 1, 0 },
  { "bake"     , "bak"	, POS_SITTING , do_assemble , 0, SCMD_BAKE },
  ...
  { "break"    , "break"	, POS_STANDING, do_break    , LVL_IMMORT, 0 },
  { "brew"     , "brew"	, POS_SITTING , do_assemble , 0, SCMD_BREW },
  ...
  { "compare"  , "comp"	, POS_RESTING , do_compare  , 0, 0 },
  { "commands" , "com"	, POS_DEAD    , do_commands , 0, SCMD_COMMANDS },
  { "compact"  , "compact"	, POS_DEAD    , do_gen_tog  , 0, SCMD_COMPACT },
  { "copyover" , "copyover"	, POS_DEAD    , do_copyover , LVL_GRGOD, 0 },
  { "craft"    , "craft"	, POS_SITTING , do_assemble , 0, SCMD_CRAFT },
  ...
  { "drive"    , "drive"	, POS_STANDING, do_drive    , 0, 0 },
  ...
  { "file"     , "fi"	, POS_SLEEPING, do_file     , LVL_GRGOD, 0 },
  { "fix"      , "fix"	, POS_STANDING, do_fix      , LVL_IMMORT, 0 },
  { "flee"     , "fl"	, POS_FIGHTING, do_flee     , 1, 0 },
  { "fletch"   , "fletch"	, POS_SITTING , do_assemble , 0, SCMD_FLETCH },
  { "follow"   , "fol"	, POS_RESTING , do_follow   , 0, 0 },
  { "forge"    , "for"	, POS_SITTING , do_assemble , 0, SCMD_FORGE },
  ...
  { "gedit"    , "gedit"      , POS_DEAD    , do_oasis    , LVL_BUILDER, SCMD_OASIS_GEDIT },
  { "gemote"   , "gem"  , POS_SLEEPING, do_gen_comm , 0, SCMD_GEMOTE },
  { "glist"    , "glist"      , POS_SLEEPING, do_oasis    , LVL_BUILDER, SCMD_OASIS_GLIST },
  ...
  { "knit"     , "knit"	, POS_SITTING , do_assemble , 0, SCMD_KNIT },

  { "look"     , "l"	, POS_RESTING , do_look     , 0, SCMD_LOOK },
  { "languages", "lang"	, POS_RESTING , do_languages, 0, 0 },
  ...
  { "links"    , "lin"	, POS_DEAD    , do_oasis    , LVL_BUILDER, SCMD_OASIS_LINKS },
  ...
  { "make"     , "make"	, POS_STANDING, do_assemble , 0, SCMD_MAKE },
  ...
  { "pilot"    , "pilot"	, POS_STANDING, do_drive    , 0, 0 },
  ...
  { "qecho"    , "que"	, POS_DEAD    , do_qcomm    , LVL_GOD, SCMD_QECHO },
  ...
  { "sacrifice", "sac"	, POS_SLEEPING, do_sac      , 0, 0 },
  ...
  { "search"   , "sea"	, POS_STANDING, do_look     , 0, SCMD_SEARCH },
  ...
  { "speak"    , "spe"	, POS_RESTING , do_languages, 0, 0 },
  ...
  { "tedit"    , "tedit"	, POS_DEAD    , do_tedit    , LVL_GRGOD, 0 },  
  { "thatch"   , "thatch"	, POS_SITTING , do_assemble , 0, SCMD_THATCH },
  ...
  { "turn"     , "turn"	, POS_STANDING, do_turn     , 0, 0},
  ...
  { "weave"    , "weave"	, POS_SITTING , do_assemble , 0, SCMD_WEAVE },
...
void display_races(struct descriptor_data *d)
{
  ...
  for (x = 0; x < NUM_RACES; x++)
    if (race_ok_gender[(int)GET_SEX(d->character)][x])
      send_to_char(d->character, race_display[x]);
...
void display_classes(struct descriptor_data *d)
{
  ...
  for (x = 0; x < NUM_CLASSES; x++)
    if (class_ok_race[(int)GET_RACE(d->character)][x])
      send_to_char(d->character, class_display[x]);
...
void nanny(struct descriptor_data *d, char *arg)
{
  ...
  case CON_PASSWORD:		/* get pwd for known player      */
	...
	write_to_output(d, "%s", motd);
      ...
      if (load_result) {
		...
		CCRED(d->character, C_BRI), load_result,
		(load_result > 1) ? "S" : "", CCNRM(d->character, C_BRI));
	GET_BAD_PWS(d->character) = 0;
      }
      write_to_output(d, "\r\n*** PRESS RETURN: ");
      STATE(d) = CON_RMOTD;
  ...
  case CON_CLASS_HELP:
    ...
    if (isdigit(*arg)) {
      player_i = atoi(arg);
      if (player_i > NUM_CLASSES || player_i < 1) {
        write_to_output(d, "\r\nThat's not a class.\r\nHelp on Class #: ");
        break;
      }
      player_i -= 1;
