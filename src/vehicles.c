 ...
 void drive_into_vehicle(struct char_data *ch, struct obj_data *vehicle, char *arg)
{
  ...
  } else if (!(vehicle_in_out = get_obj_in_list_vis(ch, arg, NULL, world[vehicle->in_room].contents)) ) {
...
void drive_outof_vehicle(struct char_data *ch, struct obj_data *vehicle)
{
  ...
  if ( !(hatch = get_obj_in_list_type(ITEM_HATCH,world[vehicle->in_room].contents)) ) {
