...
ASPELL(spell_identify)
{
  ...
  if (obj) {
    ...
    case ITEM_POTION:
      ...
      if (GET_OBJ_VAL(obj, VAL_POTION_SPELL1) >= 1) {
	i = snprintf(bitbuf + len, sizeof(bitbuf) - len, " %s", skill_name(GET_OBJ_VAL(obj, VAL_POTION_SPELL1)));
        if (i >= 0)
          len += i;
      }

      if (GET_OBJ_VAL(obj, VAL_POTION_SPELL2) >= 1 && len < sizeof(bitbuf)) {
	i = snprintf(bitbuf + len, sizeof(bitbuf) - len, " %s", skill_name(GET_OBJ_VAL(obj, VAL_POTION_SPELL2)));
        if (i >= 0)
          len += i;
      }

      if (GET_OBJ_VAL(obj, VAL_POTION_SPELL3) >= 1 && len < sizeof(bitbuf)) {
	i = snprintf(bitbuf + len, sizeof(bitbuf) - len, " %s", skill_name(GET_OBJ_VAL(obj, VAL_POTION_SPELL3)));
        if (i >= 0)
          len += i;
      }
    ...
    case ITEM_WEAPON:
      ...
      break;
    case ITEM_ARMOR:
      send_to_char(ch, "AC-apply is %d\r\n", GET_OBJ_VAL(obj, VAL_ARMOR_APPLYAC));
      break;
    }
    found = FALSE;
...
ASPELL(spell_portal)
{
  ...
  GET_OBJ_VAL(portal, VAL_PORTAL_HEALTH) = 50;
  GET_OBJ_VAL(portal, VAL_PORTAL_MAXHEALTH) = 50;
  ...
  GET_OBJ_VAL(tportal, VAL_PORTAL_HEALTH) = 50;
  GET_OBJ_VAL(tportal, VAL_PORTAL_MAXHEALTH) = 50;
