	     ...
	     struct obj_data *ovict, int spellnum, int level, int casttype)
{
  ...
  if (ROOM_FLAGGED(IN_ROOM(caster), ROOM_NOMAGIC) && GET_LEVEL(caster) < LVL_IMPL) {
    send_to_char(caster, "Your magic fizzles out and dies.\r\n");
    act("$n's magic fizzles out and dies.", FALSE, caster, 0, 0, TO_ROOM);
    return (0);
  }
  if (ROOM_FLAGGED(IN_ROOM(caster), ROOM_PEACEFUL) && GET_LEVEL(caster) < LVL_IMPL && 
  ...
  if (IS_SET(SINFO.routines, MAG_AFFECTSV))
    mag_affectsv(level, caster, cvict, spellnum, savetype);
		          ...
		          char *argument)
{
  ...
  case ITEM_STAFF:
    ...
    if (GET_OBJ_VAL(obj, VAL_STAFF_CHARGES) <= 0) {
	  ...
	  i++;
	while (i-- > 0)
	  call_magic(ch, NULL, NULL, GET_OBJ_VAL(obj, VAL_STAFF_SPELL), k, CAST_STAFF);
      } else {
	for (tch = world[IN_ROOM(ch)].people; tch; tch = next_tch) {
	  next_tch = tch->next_in_room;
	  if (ch != tch)
	    call_magic(ch, tch, NULL, GET_OBJ_VAL(obj, VAL_STAFF_SPELL), k, CAST_STAFF);
	}
      }
    }
    break;
  ...
  case ITEM_SCROLL:
    ...
    for (i = 1; i <= 3; i++)
      if (call_magic(ch, tch, tobj, GET_OBJ_VAL(obj, i),
		       GET_OBJ_VAL(obj, VAL_SCROLL_LEVEL), CAST_SCROLL) <= 0)
	break;
  ...
  if (!consume_otrigger(obj, ch, OCMD_QUAFF))  /* check trigger */
    ...
    for (i = 1; i <= 3; i++)
      if (call_magic(ch, ch, NULL, GET_OBJ_VAL(obj, i),
		       GET_OBJ_VAL(obj, VAL_POTION_LEVEL), CAST_POTION) <= 0)
	break;
...
ACMD(do_cast)
{
  ...
  struct obj_data *tobj = NULL;
  char *s, *t;
  int mana, spellnum, i, percent, target = 0;
  ...
  if ((IS_NPC(ch) && GET_MSKILL(ch, spellnum) == 0) || 
      (!IS_NPC(ch) && GET_SKILL(ch, spellnum) == 0) ) {
  ...
  if (percent < SG_MIN)
    skill_gain(ch, spellnum);

  /* You throws the dice and you takes your chances.. 101% is total failure */
  if (IS_NPC(ch)) {
    if (percent > GET_MSKILL(ch, spellnum)) {
      WAIT_STATE(ch, PULSE_VIOLENCE);
    ...
    } else { /* cast returns 1 on success; subtract mana & set waitstate */
    ...
    if (percent > GET_SKILL(ch, spellnum)) {
      WAIT_STATE(ch, PULSE_VIOLENCE);
      if (!tch || !skill_message(0, ch, tch, spellnum))
        send_to_char(ch, "You lost your concentration!\r\n");
      if (mana > 0)
...
void spell_race_level(int spell, int race, int level)
{
  int bad = 0;
  ...
  if (!bad)
    spell_info[spell].min_rlevel[race] = level;
}
	...
	int mana_change, int minpos, int targets, int violent, int routines, const char *wearoff)
{
  ...
  for (i = 0; i < NUM_RACES; i++)
    spell_info[spl].min_rlevel[i] = LVL_IMMORT;
...
void unused_spell(int spl)
{
  ...
  for (i = 0; i < NUM_RACES; i++)
    spell_info[spl].min_rlevel[i] = LVL_IMPL + 1;
...
void mag_assign_spells(void)
{
  ...
  spello(SPELL_PARALIZE, "paralize", 25, 10, 1, POS_STANDING,
	TAR_CHAR_ROOM | TAR_NOT_SELF, TRUE, MAG_AFFECTSV,
  ...
  skillo(SKILL_WP_DAGGER, "dagger");
  skillo(SKILL_WP_SWORD_SHORT, "short sword");
  skillo(SKILL_WP_SWORD_LONG, "long sword");
  skillo(SKILL_WP_SWORD_TWOHANDED, "twohanded sword");
  skillo(SKILL_WP_MACE, "mace");
  skillo(SKILL_WP_AXE, "axe");
  skillo(SKILL_WP_WHIP, "whip");
  skillo(SKILL_WP_SPEAR, "spear");
  skillo(SKILL_WP_POLEARM, "polearm");
  skillo(SKILL_WP_UNARMED, "unarmed");
  skillo(SKILL_WP_FLAIL, "flail");
  skillo(SKILL_WP_STAFF, "staff");
  skillo(SKILL_WP_HAMMER, "hammer");
  skillo(SKILL_MAKE, "make");
  skillo(SKILL_BREW, "brew");
  skillo(SKILL_BAKE, "bake");
  skillo(SKILL_ASSEMBLE, "assemble");
  skillo(SKILL_CRAFT, "craft");
  skillo(SKILL_FLETCH, "fletch");
  skillo(SKILL_KNIT, "knit");
  skillo(SKILL_MIX, "mix");
  skillo(SKILL_THATCH, "thatch");
  skillo(SKILL_WEAVE, "weave");
  skillo(SKILL_FORGE, "forge");
