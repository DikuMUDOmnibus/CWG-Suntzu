...
void damage_object(struct char_data *ch, struct char_data *victim)
{
  ...
  if (!IS_NPC(ch)) {
    ...
    if (object)
      snum = GET_MSKILL(ch, GET_OBJ_VAL(object, VAL_WEAPON_SKILL));
    else
      snum = GET_MSKILL(ch, SKILL_WP_UNARMED);
  }
...
int damage(struct char_data *ch, struct char_data *victim, int dam, int attacktype)
{
  ...
  /* peaceful rooms */
  if (GET_MOB_VNUM(ch) != DG_CASTER_PROXY && GET_LEVEL(ch) < LVL_IMPL &&
  ...
  default:			/* >= POSITION SLEEPING */
    ...
    if (GET_HIT(victim) < (GET_MAX_HIT(victim) / 4)) {
      send_to_char(victim, "%sYou wish that your wounds would stop BLEEDING so much!%s\r\n",
		CCRED(victim, C_BRI), CCNRM(victim, C_BRI));
...
void hit(struct char_data *ch, struct char_data *victim, int type)
{
  ...
  if (wielded)
    snum = (GET_OBJ_VAL(wielded, VAL_WEAPON_SKILL));
  else
    snum = SKILL_WP_UNARMED;

  if (!IS_NPC(ch))
    wskill = GET_SKILL(ch, snum);
  else
    wskill = GET_MSKILL(ch, snum);
  ...
  if (diceroll == 20 || !AWAKE(victim))
   ...
   /* weapon skill check, can cause a hit (above) to miss! */
  if (dam && (diceroll2 <= wskill))
    dam = TRUE;
  else 
    dam = FALSE;
  ...
  if (!dam)
    ...
    if (wielded && GET_OBJ_TYPE(wielded) == ITEM_WEAPON) {
      ...
      dam += dice(GET_OBJ_VAL(wielded, VAL_WEAPON_DAMDICE), GET_OBJ_VAL(wielded, VAL_WEAPON_DAMSIZE));
    } else {
