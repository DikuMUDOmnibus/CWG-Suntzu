...
void char_from_room(struct char_data *ch)
{
  ...
  if (GET_EQ(ch, WEAR_LIGHT) != NULL)
    if (GET_OBJ_TYPE(GET_EQ(ch, WEAR_LIGHT)) == ITEM_LIGHT)
      if (GET_OBJ_VAL(GET_EQ(ch, WEAR_LIGHT), VAL_LIGHT_HOURS))	/* Light is ON */
...
void char_to_room(struct char_data *ch, room_rnum room)
{
  if (ch == NULL || room == NOWHERE || room > top_of_world)
    ...
    if (GET_EQ(ch, WEAR_LIGHT))
      if (GET_OBJ_TYPE(GET_EQ(ch, WEAR_LIGHT)) == ITEM_LIGHT)
	if (GET_OBJ_VAL(GET_EQ(ch, WEAR_LIGHT), VAL_LIGHT_HOURS))	/* Light ON */
	  world[room].light++;
...
void update_char_objects(struct char_data *ch)
{
  ...
  if (GET_EQ(ch, WEAR_LIGHT) != NULL)
    if (GET_OBJ_TYPE(GET_EQ(ch, WEAR_LIGHT)) == ITEM_LIGHT)
      if (GET_OBJ_VAL(GET_EQ(ch, WEAR_LIGHT), VAL_LIGHT_HOURS) > 0) {
	i = --GET_OBJ_VAL(GET_EQ(ch, WEAR_LIGHT), VAL_LIGHT_HOURS);
	if (i == 1) {
...
void affectv_to_char(struct char_data *ch, struct affectedv_type *af)
{
  struct affectedv_type *affected_alloc;

  CREATE(affected_alloc, struct affectedv_type, 1);
...
void affectv_remove(struct char_data *ch, struct affectedv_type *af)
{
  struct affectedv_type *temp;
...
void affectv_join(struct char_data *ch, struct affectedv_type *af,
                      bool add_dur, bool avg_dur, bool add_mod, bool avg_mod)
{
  struct affectedv_type *hjp, *next;
  bool found = FALSE;
    ...
    switch (GET_OBJ_TYPE(object)) {
      case ITEM_WEAPON:
        if (!GET_EQ(ch, WEAR_WIELD)) {
          perform_wear(ch, object, WEAR_WIELD);
        } else {
          if (is_better(object, GET_EQ(ch, WEAR_WIELD))) {
