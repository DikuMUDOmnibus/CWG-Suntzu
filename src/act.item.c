...
ACMD(do_assemble)
{
  ...
  if (!GET_SKILL(ch, subcmd)) {
    send_to_char(ch, "You know nothing of that art!\r\n");
    return;
  }
  ...
  if (*argument == '\0') {
    ...
    return;
  } else if (assemblyGetType(lVnum) != (subcmd - 101)) {
    send_to_char(ch, "You can't %s %s %s.\r\n", CMD_NAME, AN(argument), argument);
    return;
  ...
  if (percent < SG_MIN)
    skill_gain(ch, subcmd);

  if (percent > GET_SKILL(ch, subcmd)) {
    send_to_char(ch, "You attempt to create ends in failure!\r\n");
    return;
  } else {
...
ACMD(do_eat)
{
  ...
  if (GET_OBJ_VAL(food, VAL_FOOD_POISON) && (GET_LEVEL(ch) < LVL_IMMORT)) {
...
int hands(struct char_data * ch)
{
  ...
  if (GET_EQ(ch, WEAR_WIELD)) {
    if (OBJ_FLAGGED(GET_EQ(ch, WEAR_WIELD), ITEM_2H)) {
      x[0] = 2;
    } else
    x[0] = 1;
  }
  ...
  if (GET_EQ(ch, WEAR_HOLD)) {
    if (OBJ_FLAGGED(GET_EQ(ch, WEAR_HOLD), ITEM_2H)) {
      x[2] = 2;
    } else
    x[2] = 1;
  }
...
void perform_wear(struct char_data *ch, struct obj_data *obj, int where)
{
  ...
  int wear_bitvectors[] = {
    ...
    ITEM_WEAR_WIELD, ITEM_WEAR_TAKE, ITEM_WEAR_PACK, ITEM_WEAR_EAR,
  ...
  if (OBJ_FLAGGED(obj, ITEM_2H) && hands(ch) > 0) {
...
ACMD(do_wield)
{
  ...
  if (!*arg)
    ...
    else if (!IS_NPC(ch) && GET_SKILL(ch, (GET_OBJ_VAL(obj, VAL_WEAPON_SKILL))) < 1 )
      send_to_char(ch, "You have no idea which end is up!\r\n");
