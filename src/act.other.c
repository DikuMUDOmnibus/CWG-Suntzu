  ...
  if (!AFF_FLAGGED(ch, AFF_GROUP))
    ...
    if (AFF_FLAGGED(k, AFF_GROUP)) {
      snprintf(buf, sizeof(buf), "     [%3dH %3dM %3dV] [%2d %s %s] $N (Head of group)",
	      GET_HIT(k), GET_MANA(k), GET_MOVE(k), GET_LEVEL(k), CLASS_ABBR(k), RACE_ABBR(k));
...
void skill_gain(struct char_data *ch, int skill_num)
{
  ...
  if (GET_LEVEL(ch) >= LVL_IMMORT) /* Immortals and up no longer need this */
    return;

  if (!IS_NPC(ch)) {
    if (GET_SKILL(ch, skill_num) > 97)
      return;
    else if (GET_MSKILL(ch, skill_num) > 97)
    return;
  }

  if (!IS_NPC(ch))
    temp = (GET_SKILL(ch, skill_num));
  else
    temp = (GET_MSKILL(ch, skill_num));
  gain = (rand_number(1, GET_INT(ch)));
  temp = temp + gain;
  ...
  if (!IS_NPC(ch))
    SET_SKILL(ch, skill_num, temp);
  else
    SET_MSKILL(ch, skill_num, temp);

  gain_exp(ch, gain);
  send_to_char(ch,"Your skill in %s improves!\r\n", spell_info[skill_num].name);
}
