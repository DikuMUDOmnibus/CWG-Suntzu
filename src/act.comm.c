...
ACMD(do_languages)
{
  ...
  if (!*arg)
    ...
    for (i = MIN_LANGUAGES; i < MAX_LANGUAGES; i++) {
      if ((search_block(arg, languages, FALSE) == i-MIN_LANGUAGES) && GET_SKILL(ch, i) > 59) {
        SPEAKING(ch) = i;
...
ACMD(do_say_languages)
{
  ...
  if(!*argument) {
    ...
    if (!IS_NPC(ch))
      ...
      garble_text(ibuf, 100, MIN_LANGUAGES);
    ...
    for (tch = world[ch->in_room].people; tch; tch = tch->next_in_room) {
      if (tch != ch && AWAKE(tch) && tch->desc) {
        ...
        if (CAN_SEE(tch, ch))
          ...
          strcpy(cbuf, "Someone");
        if (!IS_NPC(ch) && GET_SKILL(tch, SPEAKING(ch)) < 1)
	 send_to_char(tch, "%s %ss, in an unfamiliar tongue, '%s'\r\n", CAP(cbuf), verb, obuf);
        else
	 send_to_char(tch, "%s %ss '%s'\r\n", CAP(cbuf), verb, obuf);
      }
    }
...
ACMD(do_say_original)
{
  ...
  if (!*argument)
    ...
    snprintf(buf, sizeof(buf), "$n %ss, '%s'", verb, argument);
